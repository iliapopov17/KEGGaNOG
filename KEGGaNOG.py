...
def parse_emapper(input_file, temp_folder):
    ...
    df_kegg_ko['KEGG_ko'] = df_kegg_ko['KEGG_ko'].str.replace(r'ko:(K\d+)', r'SAMPLE \1', regex=True)
    ...
    df_kegg_ko.to_csv(parsed_file, sep='\t', index=False, header=False, quoting=csv.QUOTE_MINIMAL, escapechar='\\')
...
def run_kegg_decoder(input_file, temp_folder):
    ...
    subprocess.run(['KEGG-decoder', '-i', input_file, '-o', output_file, '-v', 'static'])
...
def generate_heatmap(kegg_decoder_file, output_folder, dpi, sample_name):
    ...
    # Plot each part
    sns.heatmap(df1.pivot_table(values=sample_name, index='Function', fill_value=0),
                cmap="Blues", annot=True, linewidths=.5, ax=axes[0], cbar=False)
    axes[0].set_title('Part 1')
    
    sns.heatmap(df2.pivot_table(values=sample_name, index='Function', fill_value=0),
                cmap="Blues", annot=True, linewidths=.5, ax=axes[1], cbar=False)
    axes[1].set_title('Part 2')
    
    sns.heatmap(df3.pivot_table(values=sample_name, index='Function', fill_value=0),
                cmap="Blues", annot=True, linewidths=.5, ax=axes[2], cbar_ax=cbar_ax)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="KEGGaNOG: Link eggnog-mapper and KEGG-Decoder for pathway visualization.")
    ...
    parser.add_argument('--dpi', type=int, default=600, help='DPI for the output image (default: 600)')
    parser.add_argument('--n', default='SAMPLE', help='Sample name for labeling (default: SAMPLE)')
    
    args = parser.parse_args()
    
    # Create output and temporary directories
    os.makedirs(args.o, exist_ok=True)
    temp_folder = os.path.join(args.o, 'temp_files')
    os.makedirs(temp_folder, exist_ok=True)
    
    # Step 1: Parse eggnog-mapper output
    parsed_filtered_file = parse_emapper(args.i, temp_folder)
    
    # Step 2: Run KEGG-Decoder
    kegg_decoder_file = run_kegg_decoder(parsed_filtered_file, temp_folder)
    
    # Step 3: Generate the heatmap
    generate_heatmap(kegg_decoder_file, args.o, args.dpi, args.n)
    
    print(f"Heatmap saved in {args.o}")
